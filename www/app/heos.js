function helm_r1234yf(rho, T) {
    if (T < 220 || T > 410) {
        throw new Exception('The current input temperature value is out of range for this substance correlations');
    }

    // exception handling function
    function Exception(message) {
        this.name = 'R1234yf properties exception';
        this.message = message || 'The current input values are out of range for this substance correlations';
    }
    ...
    var result = {
        ...
        h: R * T * (alpha0tal() + alphaRtal() + alphaRdelta() + 1),
        p: rho * R * T * (1 + (alphaRdelta())),
        s: R * (alpha0tal() + alphaRtal() - alpha0() - alphaR()),
        u: R * T * (alpha0tal() + alphaRtal()),
        v: 1 / rho,
        w: (R * T / M) * Math.sqrt(1 + 2 * alphaRdelta() + alphaRdeltadelta() - (Math.pow((1 + alphaRdelta() - alphaRtaldelta()), 2) / (alpha0taltal() + alphaRtaltal())))
    };

    if (result.p > 30000000) {
        throw new Exception('The resulting pressure value is out of range for this substance correlations');
    } else {
        return result;
    }
}
